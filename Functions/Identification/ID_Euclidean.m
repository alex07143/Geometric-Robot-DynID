%% %% Inertial parameter identification with Euclidean distance regularization
% 2019 Taeyoon Lee
...
%% Implementation
function [ Params, LS_error ] = ID_Euclidean( Opt_type, A, b, Phi_prior, Sigma, gamma_or_c, Q )
%% Initialization
if strcmp(Opt_type, 'regularized')
    fprintf(['Solving Euclidean distance ', Opt_type, ' identification formulation with regularization factor (gamma) = %f \n'], gamma_or_c); 
elseif strcmp(Opt_type, 'point-to-set')
    fprintf(['Solving Euclidean distance ', Opt_type, ' identification formulation with least square error bound (c) = %f \n'], gamma_or_c);
else
...
if strcmp(Opt_type, 'regularized')
    ...
    J_regularizer = sum_square( Phi(:) -Phi_prior(:) );
